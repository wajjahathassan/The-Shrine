...
from the_shrine import Shrine

from ask_shrine import ask_the_shrine
...
def ask_oracle():
    # Gets the question from the website
    data = request.get_json()
    user_question = data.get("question")

    # Asks the Caretaker (using the RAG script)
    answer, source = ask_the_shrine(user_question, chat_history)

    # Updates Memory (Saves what was just said)
    chat_history.append(f"User: {user_question}")
    chat_history.append(f"Shrine: {answer}")
